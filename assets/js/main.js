...
document.addEventListener('DOMContentLoaded', () => {
    ...
    auth.onAuthStateChanged(user => {
        if (user) {
            // Para garantir que o nome seja exibido, buscamos do Firestore se o displayName estiver vazio
            db.collection('users').doc(user.uid).get().then(doc => {
                const userData = doc.data();
                state.loggedInUser = {
                    email: user.email,
                    uid: user.uid,
                    displayName: user.displayName || (userData ? userData.name : user.email.split('@')[0])
                };
