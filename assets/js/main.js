...
function handleFavoriteToggle(event) {
    ...
    if (favoriteIndex > -1) {
        ...
        const priceEl = card.querySelector('.product-price-display');
        const price = parseFloat(priceEl.textContent.replace(/[^0-9,-]+/g, "").replace(",", "."));
        state.favorites.push({
            id: productId,
            name: card.querySelector('.product-name-display').textContent,
            price: price,
