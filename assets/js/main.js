...
document.addEventListener('DOMContentLoaded', () => {
        ...
        if (state.favorites.length === 0) {
            ...
            state.favorites.forEach(item => {
                ...
                <div class="product-card bg-white rounded-lg shadow" data-id="${item.id}" data-name="${item.name}" data-price="${item.price}" data-image="${item.image}">
                    <div class="relative">
                        <button class="favorite-btn absolute top-2 right-2 text-2xl" data-id="${item.id}"><i class="fas fa-heart text-red-500"></i></button>
                        <img src="${item.image}" class="w-full h-48 object-contain p-4">
                    </div>
                    <div class="p-4">
    ...
    async function initializeApp() {
        ...
        // Adicionar outros listeners que dependem de elementos carregados dinamicamente
