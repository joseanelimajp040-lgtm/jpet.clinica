...
// --- NOVA FUNÇÃO PARA GERENCIAR CLIENTES ---
async function renderAdminClientsView() {
    const adminContent = document.getElementById('admin-content');
    if (!adminContent) return;

    // 1. Define a estrutura HTML da página de clientes
    adminContent.innerHTML = `
        <header class="mb-8">
...
    let allClients = []; // Array para guardar a lista completa de clientes

    // 2. Função auxiliar para exibir a lista de clientes no HTML
...
            // Formata a data de criação da conta do usuário
...
    // 3. Busca os dados dos clientes do Firestore
    try {
...
        displayClients(allClients); // Exibe a lista completa inicialmente
...
            // Filtra os clientes com base no nome ou e-mail
            const filteredClients = allClients.filter(client =>
                (client.name && client.name.toLowerCase().includes(searchTerm)) ||
                (client.email && client.email.toLowerCase().includes(searchTerm))
            );

            displayClients(filteredClients); // Exibe os resultados filtrados
        });
...
async function renderAdminProductEditView(productId) {
    ...
    try {
        // 1. Busca os dados do produto específico
        ...
        // 2. Gera os campos do formulário para as variações do produto
        ...
        // 3. Monta o formulário completo com os dados do produto
        adminContent.innerHTML = `
            <header class="mb-6">
...
                        // --- LÓGICA DE NAVEGAÇÃO DO ADMIN ATUALIZADA ---
...
        if (e.target.id === 'create-account-form') handleCreateAccount(e);
        ...
        if (e.target.id === 'edit-product-form') {
            ...
            try {
                // 1. Coleta os dados gerais do produto
                const updatedData = {
                ...
                // 2. Coleta os dados de cada variação
